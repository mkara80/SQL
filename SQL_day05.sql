...
--26-UPDATE...
   --SET sütun_adi=yeni_deger
   --WHERE kosul              : kosulu saglayan satirlarda ki degerleri gunceller
...
UPDATE calisanlar3
SET isyeri=(SELECT isyeri FROM calisanlar3 WHERE isim='Veli Yildirim') WHERE isim='Ali Seker'
...
--28-ORDER BY: Belirli bir ya da daha fazla sütuna göre varsayılan olarak NATURAL(artan,ASCENDING)olarak sıralamamızı saglar
CREATE TABLE person
(
ssn char(9),
isim varchar(50),
...
SELECT isim || ' '||soyisim ad_soyad--burada ki ad_soyad ı biz verdik rumuz gibi alias ile
FROM person
