...
FROM courses 
WHERE course_name 
IN(SELECT course_name FROM teachers WHERE age < 35);
--2--Her bir şehirde verilen derslerin isimlerini ve kredisini listeleyiniz. 

SELECT city,course_name, 
(SELECT credit 
 FROM courses c
 WHERE c.course_name=t.course_name )
FROM teachers t;
...
FROM teachers 
WHERE course_name 
IN(SELECT course_name 
   FROM courses 
   WHERE start_date 
   BETWEEN '01-02-2023' AND '01-05-2023' )
