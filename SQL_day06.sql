...
FROM manav
GROUP BY isim,urun_adi-- burada iki grup var önce isme göre sonra ürün miktarına göre
ORDER BY isim
...
FROM personel
GROUP BY sirket
HAVING MIN(maas)>4000;--gruplama oldugu icin where degil bunu kullandik
...
--SORU::Her bir ismin aldıgı toplam gelir 10000 liradan fazla ise ismi ve toplam maasi gösteren sorgu yaziniz.
...
--QUESTION:==>Eğer bir şehirde çalışan personel(id) sayısı 1’den çoksa sehir ismini ve personel sayısını veren sorgu yazınız
...
--UNION ALL: UNION ile ayni fazlası tekrarli olanları da gösterir
...
INSERT INTO developers(name,email,salary,prog_lang,city,age) VALUES('Mustafa KARA','effectivelly@gmail.com',7500,'Java','Osmaniye',31);
...
--QUESTION::--Java kullananların maaşını ve prog. dilini ve 
...
UNION
SELECT age AS mass_yas,prog_lang FROM developers WHERE prog_lang='JavaScript'

--QUESTION::>>id si 8 olan developerın çalıştığı şehir ve maaşını
...
--QUESTION::>>--developers tablosundaki şehirler ve
